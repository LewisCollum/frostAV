...
To add support for the INA226 a device tree overlay needed to be created. This is a file which inserts additional data into the device tree. It attaches to a target, which is the bus or interface which the new device is attached to. In this case, the overlay is attached to I2C bus 1, and the device is defined as 
** Problems
* Test Results
