...
*** Remote Motor/Servo Shutoff
   ...
   #+caption: Failsafe Remote Schematic.
   #+attr_html: :width 300px
   #+attr_latex: :width \linewidth/2
   [[./figure/failsafe_remote.png]]
   #+caption: Failsafe Car Side Schematic.
   #+attr_html: :width 300px
   #+attr_latex: :width \linewidth/2
   [[./figure/failsafe_carside.png]]


A remote shutoff system was implemented using Xbee radio modules and an IRF250 power MOSFET. This system allows the car to be stopped remotely if it were to go out of control. The system is designed to be failsafe, so that any failure will disconnect power to the motor and servo. If the Xbee on the car side loses signal or power, it will stop the motor.
